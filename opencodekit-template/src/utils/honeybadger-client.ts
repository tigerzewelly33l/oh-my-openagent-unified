...
import { captureSentryError, initSentryClient, resetSentryStateForTest } from "./sentry-client.js";
...
const HONEYBADGER_REPORTED = Symbol("honeybadger-reported");
...
export function initHoneybadger(base: HoneybadgerBaseContext): void {
  ...
  const release =
    process.env.SENTRY_RELEASE ?? process.env.HONEYBADGER_REVISION ?? packageJson.version;
...
export async function captureHoneybadgerError(
  ...
  const notice = {
    ...
    component:
      typeof baseContext.component === "string" ? baseContext.component : undefined,
...
export function markHoneybadgerReportedError(error: Error): Error {
  Object.defineProperty(error, HONEYBADGER_REPORTED, {
    value: true,
    configurable: true,
  });
  return error;
...
export function hasHoneybadgerReportedError(error: unknown): boolean {
  return (
    typeof error === "object" &&
    error !== null &&
    HONEYBADGER_REPORTED in error
  );
}
