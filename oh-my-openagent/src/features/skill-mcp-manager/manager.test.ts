...
async function importFreshManagerModule(): Promise<typeof import("./manager")> {
	...
	const freshConnection = await import(
		`./connection?manager-connection=${Date.now()}-${Math.random()}`
	);
	mock.module("./connection", () => ({
		getOrCreateClient: freshConnection.getOrCreateClient,
		getOrCreateClientWithRetryImpl:
			freshConnection.getOrCreateClientWithRetryImpl,
	}));
	...
	const realHttpTransportModule = await import(
		"@modelcontextprotocol/sdk/client/streamableHttp.js"
	);
	const realStdioTransportModule = await import(
	...
	mock.module(
		"@modelcontextprotocol/sdk/client/streamableHttp.js",
		() => realHttpTransportModule,
	);
	mock.module(
		"@modelcontextprotocol/sdk/client/stdio.js",
		() => realStdioTransportModule,
	);
	mock.module("./connection", () => realConnectionModule);
	return module;
}

type SkillMcpManagerInstance = InstanceType<
	Awaited<ReturnType<typeof importFreshManagerModule>>["SkillMcpManager"]
>;
...
describe("SkillMcpManager", () => {
	let manager: SkillMcpManagerInstance;
	...
	describe("operation retry logic", () => {
		it("should retry operation when 'Not connected' error occurs", async () => {
			...
			const getOrCreateSpy = spyOn(
				manager,
				"getOrCreateClientWithRetryForTest",
			);
