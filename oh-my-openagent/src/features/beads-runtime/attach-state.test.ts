...
describe("beads runtime attach state", () => {
  ...
  function createContinuationDir(prefix: string, sessionID: string): string {
    ...
    writeBoulderState(
      directory,
      createBoulderState(join(directory, ".sisyphus", "plans", "task-5.md"), sessionID, undefined, directory),
    )
    return directory
  }
  ...
  test("persists bead and worktree metadata for the current top-level continuation", async () => {
    ...
    const result = await attachBeadToContinuation({
      ...
      runner: mock(async ({ command, cwd }: { command: readonly string[]; cwd: string }) => {
    ...
    expect(existsSync(getBeadsRuntimeAttachmentRegistryPath(join(directory, "worktree")))).toBe(true)
  })

  test("rejects duplicate attach for the same bead across another active continuation", async () => {
    const firstDirectory = createContinuationDir("omo-beads-first-", "ses-first")
    const secondDirectory = createContinuationDir("omo-beads-second-", "ses-second")
    const sharedWorktreeRoot = mkdtempSync(join(tmpdir(), "omo-beads-shared-root-"))
    testDirs.push(sharedWorktreeRoot)

      const runner = mock(async ({ command, cwd }: { command: readonly string[]; cwd: string }) => {
