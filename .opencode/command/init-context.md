...
Initialize repo-local project planning context files from templates.

> **Note:** This command can be run directly for new projects, or as a continuation from `/init` after core setup.
...
`/init-context` remains the single owner of creating or refreshing `project.md`, `roadmap.md`, and `state.md`. `/init` may hand users into this workflow, but it should not duplicate these file-creation steps.
