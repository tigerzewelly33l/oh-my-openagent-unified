...
1. **Ground**: Read bead artifacts (`prd.md`, `plan.md` if present); use `tilth --map --scope src/` (or `npx -y tilth --map --scope src/`) for codebase overview
2. **Calibrate**: Understand goal, constraints, and success criteria
3. **Transform**: Launch parallel research (`task` subagents) when uncertainty remains; use `tilth <symbol> --scope src/` (or `npx -y tilth <symbol> --scope src/`) for fast codebase discovery; decompose into phases/tasks with explicit dependencies
...
**Code navigation:** Main-agent work should prefer Tilth MCP when available; use tilth CLI for subagents, shell usage, and `--map` structural overview — see `tilth-cli` skill.
