...
`,
	...
	it("omits conflicting builtin workflow commands in OCK bead-first repos", () => {
		// given
		const ockProjectDir = join(tempDir, "ock-project");
		const commandDir = join(ockProjectDir, ".opencode", "command");

		mkdirSync(join(ockProjectDir, ".beads"), { recursive: true });
		...
		for (const commandName of [
			...
			writeFileSync(join(commandDir, commandName), `# ${commandName}\n`);
		}

		// when
		const commandNames = discoverCommandsSync(ockProjectDir).map(
			(command) => command.name,
		);

		// then
		expect(commandNames).not.toContain("start-work");
		expect(commandNames).not.toContain("ralph-loop");
		expect(commandNames).not.toContain("ulw-loop");
		expect(commandNames).not.toContain("cancel-ralph");
		expect(commandNames).not.toContain("stop-continuation");
		expect(commandNames).toContain("create");
	});

	it("prioritizes primary workflow commands ahead of support and compatibility commands", () => {
		// given
		const commandsDir = join(projectDir, ".opencode", "commands");

		mkdirSync(commandsDir, { recursive: true });
		writeFileSync(
			join(commandsDir, "lfg.md"),
			`---
description: Compatibility full-cycle command
---
Compatibility full-cycle adapter.
`,
		);
		writeFileSync(
			join(commandsDir, "compound.md"),
			`---
...
`,
		...
		writeFileSync(
			join(commandsDir, "create.md"),
			`---
description: Primary workflow command
---
Create bead-backed work.
`,
		);
		writeFileSync(
			join(commandsDir, "verify.md"),
			`---
description: Support command
---
Verify bead-backed work.
`,
		);

		// when
		const names = discoverCommandsSync(projectDir)
			.map((command) => command.name)
			.filter((name) => ["create", "verify", "lfg", "compound"].includes(name));

		// then
		expect(names).toEqual(["create", "verify", "lfg", "compound"]);
	});

	it("prefers user workflow support commands over global opencode collisions in OCK bead-first repos", () => {
		// given
		const ockProjectDir = join(tempDir, "ock-support-project");
		const markerCommandDir = join(ockProjectDir, ".opencode", "command");
		...
		mkdirSync(join(ockProjectDir, ".beads"), { recursive: true });
		mkdirSync(markerCommandDir, { recursive: true });
		...
		for (const commandName of [
			...
			writeFileSync(join(markerCommandDir, commandName), `# ${commandName}\n`);
		}
		writeFileSync(
...
`,
		...
		// when
		const verifyCommand = discoverCommandsSync(ockProjectDir).find(
			(command) => command.name === "verify",
		);

		// then
		expect(verifyCommand?.scope).toBe("user");
		expect(verifyCommand?.content).toContain("Use user verify command.");
	});

	it("prefers bead-backed handoff over builtin handoff in OCK bead-first repos", () => {
		// given
		const ockProjectDir = join(tempDir, "ock-handoff-project");
		const commandDir = join(ockProjectDir, ".opencode", "command");
...
`,
		...
		// when
		const handoffCommand = discoverCommandsSync(ockProjectDir).find(
			(command) => command.name === "handoff",
		);

		// then
		expect(handoffCommand?.scope).toBe("opencode-project");
		expect(handoffCommand?.content).toContain(
			"Use bead-backed handoff command.",
		);
	});
});
